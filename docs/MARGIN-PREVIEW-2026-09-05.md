...
## Merged bridge drill (approved in voice review)
The separate segment panel is now merged into the component section under the waterfall. Price and Cost use the same signed visual bars, with Customer → SKU or Feed category → Customer → SKU expansion. Parent effects sum original matched customer/SKU contributions on the selected bridge denominator; expanding does not rebase them. Parent before/after rates are tonnage-weighted observed averages, explicitly not the isolated rate effect when basket weights change. SKU category uses the master category carried with the source rows.

Customer/Product Mix use share markers and before/after margins in the same table. Their default customer/SKU views preserve explicit interaction adjustments; the eight alternative segment views remain selectable and explicitly do not reconcile to the selected component. Combined mix remains accessible when the canonical split is unstable. Cost retains ingredient → SKU/plant RM detail. One Region filter in the top shared controls updates the entire analysis and breadcrumbs, avoiding a duplicate bridge filter.
