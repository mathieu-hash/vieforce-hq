...
> - **Cost field — CONFIRMED 2026-06-05:** component cost = **`OITM.LastPurPrc`** (last purchase price). `AvgPrice` is 0 in `Vienovo_Live`. Verified real: CORN YELLOW ₱22.7/kg, SOY BEAN MEAL ₱29.516/kg. Component cost = `WOR1.IssuedQty × OITM.LastPurPrc`.
> - **Validated classification:** RM/Formula = ItmsGrpCod IN (101 RAW MATERIALS, 102 BASEMIX); Packaging = 104; **Feedtag = 104 AND ItemCode LIKE 'FT%'**; 105 trading-import = no production order.
> - **Phase-2 method (per-SKU COGS split):** for each sold finished good, compute its production cost ratio RM%/Pkg%/Feedtag% = Σ(IssuedQty×LastPurPrc) by class ÷ total, over a trailing window; apply that ratio to the invoice-line moving-avg COGS (revenue−GrssProfit) in the bridge per-item rows. **Ingredient contribution** = per RM component (CORN/SOY/etc.) Σ(IssuedQty×LastPurPrc), Δ period-over-period. NOT YET WIRED (SAP flapping 2026-06-05) — do NOT ship unvalidated cost numbers; validate the aggregate split reconciles to sold-COGS (~₱2,160M Jan-May) before activating.
